function PreProcess([string] $framework, [string] $nameSpace, [string]$sourceFile, [string]$destinationFile )
{
	...
	{
		if($line.Contains("namespace LightInject.ServiceLocation"))
		{
			$line = $line.Replace('LightInject.ServiceLocation', $nameSpace)			
		}
...
PreProcess "NET" "`$rootnamespace`$" $sourcepath "$scriptpath\package\content\net40\LightInjectServiceLocator.cs.pp"
PreProcess "NET" "`$rootnamespace`$" $sourcepath "$scriptpath\package\content\net45\LightInjectServiceLocator.cs.pp"
