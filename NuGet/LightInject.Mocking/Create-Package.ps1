function PreProcess([string] $framework, [string] $nameSpace, [string]$sourceFile, [string]$destinationFile )
{
	...
	{
		if($line.Contains("namespace LightInject.Annotation"))
		{
			$line = $line.Replace('LightInject.Annotation', $nameSpace)			
		}
		...
		{
			if($line.Contains("public class") -or $line.Contains("internal class") -or $line.Contains("internal static class"))
			{
...
$sourcepath = resolve-path "$scriptpath../../../LightInject.Mocking/LightInjectMocking.cs"


PreProcess "NET" "`$rootnamespace`$" $sourcepath "$scriptpath\package\content\net40\LightInjectMocking.cs.pp"
PreProcess "NET" "`$rootnamespace`$" $sourcepath "$scriptpath\package\content\net45\LightInjectMocking.cs.pp"


pushd $scriptpath

.\..\Nuget.exe pack package/LightInject.Mocking.nuspec -OutputDirectory "..\"
