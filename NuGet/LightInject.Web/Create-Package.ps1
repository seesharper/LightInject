function PreProcess([string] $framework, [string] $nameSpace, [string]$sourceFile, [string]$destinationFile )
{
	...
	{
		if($line.Contains("namespace LightInject.Web"))
		{
			$line = $line.Replace('LightInject.Web', $nameSpace)			
		}
		
		if($line.Contains('[assembly: System.Web.PreApplicationStartMethod(typeof(LightInject.Web.LightInjectHttpModuleInitializer), "Initialize")]'))
		{
			$line = '[assembly: System.Web.PreApplicationStartMethod(typeof(' + $nameSpace + '.LightInjectHttpModuleInitializer), "Initialize")]'
		}
...
$sourcepath = resolve-path "$scriptpath../../../LightInject.Web/LightInjectWeb.cs"

PreProcess "NET" "`$rootnamespace`$" $sourcepath "$scriptpath\package\content\net40\LightInjectWeb.cs.pp"
PreProcess "NET" "`$rootnamespace`$" $sourcepath "$scriptpath\package\content\net45\LightInjectWeb.cs.pp"

pushd $scriptpath

.\..\Nuget.exe pack package/LightInject.Web.nuspec  -OutputDirectory "..\"
